/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PreferencesWhereUniqueInput } from "./PreferencesWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PreferencesUpdateInput } from "./PreferencesUpdateInput";

@ArgsType()
class UpdatePreferencesArgs {
  @ApiProperty({
    required: true,
    type: () => PreferencesWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PreferencesWhereUniqueInput)
  @Field(() => PreferencesWhereUniqueInput, { nullable: false })
  where!: PreferencesWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PreferencesUpdateInput,
  })
  @ValidateNested()
  @Type(() => PreferencesUpdateInput)
  @Field(() => PreferencesUpdateInput, { nullable: false })
  data!: PreferencesUpdateInput;
}

export { UpdatePreferencesArgs as UpdatePreferencesArgs };
